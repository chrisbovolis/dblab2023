...
from dbdemo.op_review.forms import ReviewForm,BookidForm,ReviewShowForm
from dbdemo.op_review import op_review

@op_review.route("/op/reviews/<username>", methods = ["GET", "POST"])
def getreviews(username):
    ...
    cur.execute("SELECT * FROM review r INNER JOIN user u ON u.userid = r.userid WHERE u.schoolid = {};".format(usersc))   
    ...
    return render_template("op_reviews.html",username=username, reviews = reviews, pageTitle = "Search results", form=form)
    
    

@op_review.route("/op/reviews/create/<username>", methods = ["GET", "POST"]) ## "GET" by default
def createreview(username):
    """
    ...
    if(request.method == "POST" and form.validate_on_submit()):
        ...
        cur.execute("SELECT schoolid FROM user WHERE userid = {};".format(form['userid'].data))
        usersc = cur.fetchone()[0]
        cur.execute("SELECT schoolid FROM operator WHERE username = '{}';".format(username))
        opsc = cur.fetchone()[0]
        if(usersc == booksc and usersc == opsc):
            ...
            bookid = cur.fetchone()
            query = "INSERT INTO review (reviewid, text, bookid, userid, scale ) VALUE (null,'{}', '{}','{}','{}');".format(newreview['text'].data,bookid[0],form['userid'].data,newreview['scale'].data)
            try:
                cur.execute(query)
    ...
    return render_template("create_review_op.html", username =username ,pageTitle = "Create review", form = form)

@op_review.route("/op/reviews/update/<int:reviewid>", methods = ["POST"])
def updatereview(reviewid):
    ...
    if(form.validate_on_submit()):
        ...
        try:
            cur = db.connection.cursor()
            cur.execute("SELECT o.username FROM operator o INNER JOIN user u ON u.schoolid = o.schoolid INNER JOIN review r ON r.userid = u.userid WHERE r.reviewid = {}".format(reviewid))
        ...
        for category in form.errors.values():
            for error in category:
                flash(error, "danger")
    return redirect(url_for("op_review.getreviews",username= userid))

@op_review.route("/op/reviews/delete/<int:reviewid>", methods = ["POST"])
